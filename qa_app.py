    ...
    if uploaded_files:
       ...
       # Display the question-answer pairs in the sidebar with smaller text
        ...
        if user_question:
            ...
            st.write("Answer by ziyan:", answer)
            qa_col = mydb["qa"]
            qa_col.insert_one({'question':user_question, 'answer':answer  })
        
        qa_col = mydb["qa"]
        print("SDSDSDSD")
        for x in qa_col.find({},{ "_id": 0, "question": 1, "answer": 1 }):
            print(x.get('question'))
            print(x['answer'])
            st.write("quesyion", x.get('question'))
            st.write("answer",x.get('answer'))

        



        # st.write(localStorage.getItem(question))
...
def connectDB():
    ...
    try:
        ...
        mydb = client["qurangpt"]
        # mycol = mydb["qa"]
        # mydict = { "question": "test", "ans": "test" }
